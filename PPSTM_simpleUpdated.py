...
# ***** System information: *****
#
ppstm_path = '/home/foxcatcher/PycharmProjects/PPSTM/'      # path (absolute or relative) to your PPSTM code #
#
...
# ***** Main informations ******
#
scan_type     = 'didv'       # 'didv'='dIdV''='didv-single' -- only dIdV for one voltage = V ; 'v-scan'='V-scan'='Voltage-scan' -- both STM & dIdV scan - V .. Vmax; 'STM'='STM-single' -- STM for one Voltage = V, use V-scan rather; "states"="STATES" -- dIdV scancs at the eigenenergies, beware it still uses all other states for calculations #
...
V             =  0.02130946            #  -0.57116592, -0.53987516,  0.02130946,  0.05179062,  0.06449423    0.24013161  !!!! V = Vmin for SCAN !!!! #
V_max         =  0.2         # V = V_min >= -2.0 V ; V_max <= 2.0 V (othervise changes in the later code needed) #
...
dft_code      = 'sisl'       # 'fireball'='Fireball'='FIREBALL' ; 'aims'='AIMS'='FHI-AIMS' ; 'cp2k'='CP2K' ; 'gpaw'='GPAW'; 'molden'='MOLDEN'; 'dyson' ; 'sisl'= 'SISL' #
geometry_file = 'input_plot.xyz' # E.G. 'input.xyz' , 'input.bas' , 'geometry.in'; None for GPAW #
pbc           =  (0,0)       # (0,0) = None = False -- only original geometry ; (0.5,0.5) -- 2x2 cell ; (1,1) -- 3x3 cell (around original) ; (2,2) -- 5x5 cell (around original) ... #
lvs           =  None        #,[0,0,50]]      # None ; [[ax,ay,0],[bx,by,0]],[0,0,cz]] or [[ax,ay],[bx,by]] ; 'input.lvs' -- files with specified cell ; in FHI-AIMS & GPAW allready specified with geometry #
...
moldenName    = 'before.input'
imaginary = False
diegoName = 'dyson'
#
# ***** Informations for x,y,z tip_type = 'fixed' ******

x = [ -10.0, 55.0, 0.1 ]     # [xmin, xmax, dx] #
y = [ -5.0,  15.0, 0.1 ]     # [ymin, ymax, dy] #
z = [  31.0, 31.0, 1.0 ]      # !!!! z-starts from zero - normally zmin >= 3-4 Ang above heighest atoms !!!! [zmin, zmax, dz] ; for single height scan use : [z, z, 1.0] #
...
constantCurrent = False       # True / False -- write special WSxM files to be used in constant-current mode. #
XSF  = False                  # True / False -- write ".xsf" files with 3D stacks of data . For this option you have to have "installed" PPAFM in your PPSTM directory #
...
#
WorkFunction =  5.0          # 5.0 eV is standard #
fermi        = None          # None=0.0 -- no change to the Fermi Level ; -0.1 -- shifts the Fermi Level by 0.1 eV lower ... #
cut_min      = -1.5          # cut out all orbitals lower than  -2.5 eV bellow Rermi (should be: cut_min <= Vmin-2*eta) . taken to the Fermi Level #
cut_max      = +1.5          # cut out all orbitals higher than -2.5 eV above  Fermi (should be: cut_max >= Vmax+2*eta) . taken to the Fermi Level #
files_path   = '/home/foxcatcher/Desktop/7AGNRs/gatingWithSIESTA/gatednShifted/'

#/home/foxcatcher/Desktop/PPSTM/pingo/3layers/'            # where are files fron DFT code ; rather do not use this #
#
##########################################################################################################################
#                                                                                                                        #
...
import pyPPSTM.molden            as MOL
import pyPPSTM.dyson             as DY
import pyPPSTM.sisl              as SI
import matplotlib
...
if (tip_orb == 's'):
    ...
    tc = [0.15,0.425,0.425,0.,0.,0.,0.] # [s, px, py, pz, dz2, dxz, dyz ]
elif (tip_orb == 'pz'):
    ...
    print("Don't know what kind od tip you mean. I rather exit.") ; exit()
...
if (plot_atoms):
    geom_plot, tmp1, tmp2 = Bu.loadAtoms(files_path+'input_plot.xyz'); del tmp1, tmp2;
...
if ((tip_type =='relaxed') or (tip_type == 'r')):
    ...
    path_pos='/home/foxcatcher/Desktop/Mount/shayaned/circleTriangulene/pavel/ring1_Ag1L/chiral/afm/Q0.00K0.25/'
    print(path_pos)
...
if ((dft_code == 'fireball') or (dft_code == 'Fireball') or (dft_code == 'FIREBALL') or (dft_code == 'cp2k') or(dft_code == 'CP2K')):
    if isinstance(lvs, (list, tuple, np.ndarray)):
        ...
        cell = np.loadtxt(files_path+lvs, dtype=None)
        print("cell", cell)
...
#    eigEn, coefs, Ratin = RS.read_FIREBALL_all(name = files_path + 'phik_0001_', geom=files_path+geometry_file, lvs = cell, fermi=fermi, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max,cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs);
    eigEn, coefs, Ratin = RS.read_FIREBALL_all(name = files_path + 'phik_', geom=files_path+geometry_file, lvs = cell, fermi=fermi, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max,cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs);
    ...
    if ((spin == None) or (spin == False)):
        name = 'KS_eigenvectors.band_1.kpt_2.out'
    ...
    eigEn, coefs, Ratin, origLvec = RS.read_AIMS_all(name = files_path + name , geom= files_path + geometry_file, fermi=fermi, orbs = sample_orbs, pbc=pbc,imaginary=imaginary ,cut_min=cut_min, cut_max=cut_max, cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs);
    if (spin == 'both'):
...
elif ((dft_code == 'molden') or(dft_code == 'MOLDEN')): # C-atom.molden.input
    if ((spin == None)or(spin == False)):
        eigEn, coefs, Ratin = MOL.readMoldenAll(name=files_path + moldenName, fermi=None, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max, cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs, spin = "closedShell")
    elif ((spin == 'up')or(spin == 'alpha')):
        eigEn, coefs, Ratin = MOL.readMoldenAll(name=files_path + moldenName, fermi=None, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max, cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs, spin='alpha')
    elif ((spin == 'down')or(spin == 'beta')or(spin == 'dn')):
        eigEn, coefs, Ratin = MOL.readMoldenAll(name=files_path + moldenName, fermi=None, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max, cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs, spin='beta')
    elif (spin == 'both'):
        eigEn1, coefs1, Ratin = MOL.readMoldenAll(name=files_path + moldenName, fermi=None, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max, cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs, spin='alpha')
        eigEn2, coefs2, Ratin = MOL.readMoldenAll(name=files_path + moldenName, fermi=None, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max, cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs, spin='beta')
        eigEn = np.concatenate((eigEn1, eigEn2), axis=0)
        coefs = np.concatenate((coefs1, coefs2), axis=0)
    else:
        print("MOLDEN calculations failed, check the spin info. Peace."); exit()

elif ((dft_code == "dyson") or (dft_code == "Dyson")):
#    Ratin = DY.getDysonGeom(files_path+'input_plot.xyz')
    eigEn, coefs, Ratin = DY.readDysonAll(name = files_path+diegoName+".dat", geom = files_path + geometry_file, lvs=None)

elif ((dft_code == "sisl") or (dft_code == "SISL")):
    eigEn, coefs, Ratin = SI.readSislAll(geom = files_path + geometry_file, fEnz = files_path + 'enz2.dat', fn = files_path + 'coefs.npy', lvs = None)
...
#SHY
mat0 = genfromtxt(files_path+'input_plot.xyz', skip_header=2)
#data = pd.read_csv('input_plot.xyz', error_bad_lines=False)ss
data, target = np.array_split(np.loadtxt(files_path+'input_plot.xyz', skiprows=2, dtype=str), [-1], axis=1)
...
if plot_bonds:
    bonds = Bu.findBonds(xs, ys, zs, 1.59)
#    print("DEBUG: bonds are: ", bonds)

cols = Bu.pltcolor(data[:,0])
...
#            print("NoH", NoH)
            ...
            heightz = z[0]+k*z[2]
            name_plot=namez[vv]+'_height:'+str(heightz)+';tip:'+tip_type+';'+tip_orb
            if didv_b :
                # ploting part here:
                plt.figure( figsize=(0.5 * lvec[1,0] , 0.5 * lvec[2,1] ) )
                if plot_bonds:
                    Bu.plotBonds(xs, ys, bonds)
                plt.imshow(didv[vv,k,:,:], origin='lower', extent=extent , cmap='gist_heat')
                plotGeom(atoms=geom_plot)
                plt.xlabel(r' Tip_x $\AA$')
                plt.ylabel(r' Tip_y $\AA$')
                plt.title("dIdV:"+name_plot)
                plt.savefig('didv_'+namez[vv]+'_height:'+str(heightz)+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'_%03d.png' %k , bbox_inches='tight' )
              #  plt.savefig('imag_'+str(imaginary)+'didv_'+namez[vv]+'_height:'+str(heightz)+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'_%03d.png' %k , bbox_inches='tight' )
                ##plt.savefig( files_path+'didv_'+namez[vv]+'_height:'+str(heightz)+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'_%03d.png' %k , bbox_inches='tight' )
                plt.close()
            if STM_b :
                # ploting part here:
                plt.figure( figsize=(0.5 * lvec[1,0] , 0.5 * lvec[2,1] ) )
                ...
                plt.savefig( 'STM_'+namez[vv]+'_height:'+str(heightz)+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction)+"_WF_decay_"+str(round(WF_decay,1))+"_eta_"+str(eta)+'_%03d.png' %k , bbox_inches='tight' )
                plt.close()
    print("Everything plotted!")

if WSxM :
    print("Writing regular WSxM files...")
    for vv in range(NoV):
        print("DEBUG: voltage = ", vv)
        for k in range(NoH):
            print("DEBUG: height = ", k)
            if didv_b :
                name_file =  str(imaginary)+'didv_'+namez[vv]+"_height:"+str(heightz)+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'_%03d.dat' %k
                ...
                print("", file=f)
                Bu.writeMatrix(f, out_curr)
                f.close()
        #
            if STM_b :
                name_file =  'STM_'+namez[vv]+"_height:"+str(heightz)+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction)+"_WF_decay_"+str(round(WF_decay,1))+"_eta_"+str(eta)+'_%03d.dat' %k 
...
if constantCurrent :
    ...
    outZ = []
    outCur = []
    print("Writing special WSxM files for constant-current mode...")
    for vv in range(NoV):
        print(NoV)
        for k in range(NoH):
            heightz = z[0]+k*z[2]
            #name_file =  'didv_'+namez[vv]+"_"+str(heightz)+"Ang_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'_%03d.dat' %k
            name_file =  'constantCurrent_'+namez[vv]+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'.dat'
            tmp_curr=didv[vv,k,:,:].flatten()
            out_curr=np.zeros((len(tmp_curr),4))
           # print ("DEBUG = ",len(out_curr[:,0]))
            #out_curr[:,0]=tip_r0[k,:,:,0].flatten()
            #out_curr[:,1]=tip_r0[k,:,:,1].flatten()
            #out_curr[:,2]= [heightz for zz in np.arange(len(out_curr[:,0]))]
            #out_curr[:,3]=tmp_curr.copy()
            outX.append(tip_r0[k,:,:,0].flatten())
            outY.append(tip_r0[k,:,:,1].flatten())
            outZ.append([heightz for zz in np.arange(len(out_curr[:,0]))])
            outCur.append(tmp_curr)
            print(len(outX))
        #print("k is = ", k)
        #print("outX = ", Bu.flattenLst(outX))
        tempX = Bu.flattenLst(outX)
        concur = np.zeros((len(tempX),4))
        print("tempX = ", len(tempX))
        print(np.shape(outX))
        concur[:,0]=Bu.flattenLst(outX)
        concur[:,1]=Bu.flattenLst(outY)
        concur[:,2]=Bu.flattenLst(outZ)
        concur[:,3]=Bu.flattenLst(outCur)
        f=open(name_file,'w')
        print("WSxM file copyright Nanotec Electronica", file=f)
        print("WSxM ASCII XYZ file; obtained from dIdV code by Krejci et al.", file=f)
        print("X[A]  Y[A]  Z[A]", file=f)
        print("", file=f)
        Bu.writeMatrix(f, concur)
        f.close()
        #print("didv[vv]", didv[vv])
        #print("didv shape = ", didv[vv].shape)
...
#print("FUNCTION ORIG VEC = ", origVec(cell))
#origLvec = origVec(cell)

if XSF :
    print("writing XSF files...")
    #xsf_head = Bu.At2XSF(name_file, geom_plot, lvec, origLvec = origVec(cell), ELEMENTS=elements.ELEMENTS) if plot_atoms else GU.XSF_HEAD_DEFAULT
    for vv in range(NoV):
        if didv_b :
            name_file =  'didv_'+namez[vv]+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'.xsf'
            xsf_head = Bu.At2XSF(name_file, geom_plot, lvec, origLvec=origVec(cell), ELEMENTS=elements.ELEMENTS) if plot_atoms else GU.XSF_HEAD_DEFAULT
            #Bu.At2XSF(name_file, geom_plot, lvec, origLvec, ELEMENTS=elements.ELEMENTS)
            GU.saveXSF(name_file, didv[vv], lvec )
...
#    print("lvec0", lvec[0])
    #print("lvs",lvs)
    #Bu.saveGeomXSF( fname,elems,xyzs, lvs, convvec=None, bTransposed=False ):
