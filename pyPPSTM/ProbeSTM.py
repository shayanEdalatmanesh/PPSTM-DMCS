...
#    assert (NoOrb == len(coes)), "Different eigennumbers, than basis"
 ...
 #       assert (NoOrb == len(coes)*len(coes[0])/(orb_t*NoAt)), "Different eigennumbers, than basis"
    print("sh = ", sh)
    print("types: NoAt, NoOrb, Npoints, cur_1d, sh \n")
    print(type(NoAt),type(NoOrb),type(Npoints), type(cur_1d), type(sh))
...
def dIdV_sp_sp( V, WF, eta ,eig, R, Rat, coes, tip_coes, orb_t):
    ...
    print("cur1d", cur_1d)
    eig2 = np.array(eig, dtype=float)
    print("len cur",  len(cur_1d))
#    np.array(ast.literal_eval(b), dtype=float)
#    eig2 = np.ndarray(eig)
    print("type eig2", type(eig2))
    print(eig2)
    print("type Rat", type(Rat))

#    eig = np.ndarray(eig)
    lib.proc_dIdVspdspd( orb_t, NoAt, NoOrb, Npoints, V, WF, eta, eig2, R.copy(), Rat, coes, tip_coes, cur_1d)
