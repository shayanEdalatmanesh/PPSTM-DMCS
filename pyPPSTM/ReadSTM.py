...
def read_AIMS_coefs(fname, at_nums, imaginary ):
    #eigs = ReadSTM.getAimsEigenE(fname)
    #nMO = len(eigs)
    periods = np.array([ elements.ELEMENTS[iZ][2] for iZ in at_nums ], dtype=np.int32)
    print("PERIODS ",periods)
    print("periods type = ", type(periods))
    coefs = np.zeros(( 2*(n_max_ - n_min_) ,num_at_ , Ynum_) ); # print "DEBUG: coefs.shape", coefs.shape
    lib.read_AIMS_coefs( str.encode(fname),coefs, periods, n_max_, n_min_ , num_at_ , Ynum_ );
    print(len(coefs))
    print(np.shape(coefs))
    print(type(coefs))

    print("READ AIMS COEFS = ", coefs)
    
    print("imaginary", imaginary)
    coefsRe = coefs[::2]
    coefsIm = coefs[1::2]

# need to edit the c part to get the k points
    print("coefsRe = ", coefsRe)
    print("coefsIM = ", coefsIm)

# read the kpoints and do the exp(ikr) in C?
    return coefsRe.copy(), coefsIm.copy() #, eigs.copy()
...
def initial_check(orbs = 'sp', pbc=(0,0), imaginary = False, cut_min=-15.0, cut_max=5.0, cut_at=-1, lower_atoms=[], lower_coefs=[]):
    '''
 ...
 #   assert (imaginary == False), "sorry imaginary version is under development"
    print("imaginary mode = ", imaginary)
...
def get_AIMS_geom(geom='geometry.in'):
    ...
    '''
    print(" # ============ define atoms ============")
    atoms, nDim, lvs, origLvec = bU.loadGeometryIN(geom)
    print("atoms ",atoms)
    lvs = np.array(lvs)
    del nDim
    atoms = cut_atoms(atoms)
    ...
    for i in atoms[0]:
        ...
        #print("DEBUG at num = ", at_num)
    print(" Number of atoms: ", num_at_)
    Ratin = for_PBC(atoms,lvs)
    print("atomic geometry read")
    print("RATIN = ", Ratin)
    print(len(Ratin))
    print(np.shape(Ratin))
    return Ratin, np.array(at_num), origLvec;
...
def cut_eigenenergies(eig):
    ...
    print("cut min = ",cut_min_)
    print("cut max = ",cut_max_)

    print("n min = ",type(n_min_))
    print("n max = ",type(n_max_))
...
def	handle_coef(coef):
    ...
    #print("coef handled1: ", coef)
    coeffs = lower_Dorb(coef)
    coeffs = remove_coeffs(coeffs)
    return pbc_coef(coeffs);

# procedures for preparing everything for STM:
#def readMolden(name = 'molden.input', fermi=None, orbs = 'sp', pbc=(1,1), imaginary = False, cut_min=-15.0, cut_max=5.0, cut_at=-1, lower_atoms=[], lower_coefs=[]):
...
def	read_AIMS_all(name = 'KS_eigenvectors.band_1.kpt_1.out', geom='geometry.in', fermi=None, orbs = 'sp', pbc=(0,0), imaginary = False, cut_min=-15.0, cut_max=5.0, cut_at=-1, lower_atoms=[], lower_coefs=[]):
    '''
    ...
    Ratin, at_num, origLvec = get_AIMS_geom(geom=geom)
    #print "at_num:",at_num

    #kp = readKpintsAims(name)
    #print("K-point = ", kp)
    ...
    pre_eig=np.delete(pre_eig,[0,1,2],0) # remove the unwanted stuff
   # print("pre_eig again = ", pre_eig)
    n_bands = len(pre_eig)
    print("no. bands = ", n_bands)
    ...
    print("Eigen shape", np.shape(eig))
    print(np.shape(eig))
    # ****** TO BE REMOVED ********
    """
    ...
    coefRe, coefIm = read_AIMS_coefs(name, at_num, imaginary) # Maximal error between C++ and python reading ~1.5E-08; reading procedure ~100x faster
#    print("Coef directly from aims = ", coef)

    print(len(coefRe))
    #print("shape coef = ", np.shape(coefRe))
    #print("Coef size from aims = ", len(coefRe[0]))
    #print("Coef directly from aims 0 = ", coef[0])
    coeffs = handle_coef(coefRe)
    #print("All coefficients read")
    #print("EIGZ = ", eig )
    #print("coeffs = ", coeffs)
    #print("ratin sh = ", Ratin)


    return eig.copy(), coeffs.copy(), Ratin.copy(), origLvec;
